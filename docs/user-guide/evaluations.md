...
The Freya evaluation framework (`freya-evals`) measures model **correctness and accuracy** on academic datasets. It is a separate package from the main Freya library and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    Freya has two distinct measurement systems that complement each other:

    | System | Package | Measures | Entry Point |
    |--------|---------|----------|-------------|
    | **Evaluations** | `freya-evals` | Correctness on academic datasets (accuracy, pass rate) | `freya-eval` |
    | **Benchmarks** | `freya` | Engine performance (latency, throughput) | `freya bench` |
...
The evaluation framework is a standalone package in the `evals/` directory. Install it alongside Freya:

```bash
uv sync --extra eval
```

This installs the `freya-eval` CLI entry point and all required dependencies (`datasets`, `huggingface-hub`, `tqdm`, `rich`).
...
These benchmarks evaluate models on practical tasks that mirror real Freya use cases.
...
```bash
uv run python -m freya.evals --config src/freya/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run python -m freya.evals --config src/freya/evals/configs/use_case_v2_local.toml
```
...
|---------|-----|-------------|
| **freya-direct** | `freya-direct` | Engine-level inference via `SystemBuilder`. Works for local (Ollama, vLLM, llama.cpp) and cloud models. |
| **freya-agent** | `freya-agent` | Agent-level inference with tool calling. Uses `FreyaSystem.ask()` with the specified agent and tools. |

Use `freya-direct` for most evaluations. Use `freya-agent` when the benchmark requires tool use — for example, GAIA tasks that reference files that must be read with `file_read`, or arithmetic tasks that benefit from `calculator`.
...
| `--backend` | | choice | `freya-direct` | `freya-direct` or `freya-agent` |
| `--model` | `-m` | str | required* | Model identifier (e.g., `qwen3:8b`, `gpt-4o`) |
| `--engine` | `-e` | str | auto | Engine key (`ollama`, `vllm`, `cloud`, ...) |
| `--agent` | | str | `orchestrator` | Agent name for `freya-agent` backend |
...
```bash
freya-eval run-all -m qwen3:8b

# With options
freya-eval run-all -m gpt-4o -n 100 --output-dir results/gpt4o/
```
...
```bash
freya-eval summarize results/supergpqa_qwen3-8b.jsonl
```
...
```bash
freya-eval run --config src/freya/evals/configs/full-suite.toml
```
...
| `backend` | str | `"freya-direct"` | Inference backend: `freya-direct` or `freya-agent` |
...
| `agent` | str | `None` | Agent name for `freya-agent` backend (e.g., `"orchestrator"`) |
| `tools` | list[str] | `[]` | Tool names for `freya-agent` backend |
...
freya-eval run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Agents](agents.md) — Configure the `OrchestratorAgent` used by `freya-agent` backend
- [Tools](tools.md) — Available tools for agent-backed evaluations
- [Python SDK](python-sdk.md) — Programmatic access to Freya inference and agents
