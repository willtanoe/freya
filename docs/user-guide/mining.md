...
Freya can mine the Pearl Proof-of-Useful-Work chain through local LLM
...
Freya falls back to building from the pinned Pearl source checkout. First
...
- `freya mine models` lists Pearl model support status.
- `freya mine inspect-model` checks a Pearl model artifact before GPU launch.
- `freya mine doctor` prints hardware, Docker, Pearl node, wallet, provider,
  and session checks.
- `freya mine init` writes the local mining config and resolves the image.
- `freya mine start` launches the Pearl miner container and writes the runtime
  sidecar.
- `freya mine stop` stops the provider and removes the sidecar.
- `freya mine status` reads live gateway metrics.
- `freya mine attach` writes a sidecar for a miner you launched manually.
- `freya mine logs` prints the Docker container log tail.
- `freya mine validate-model` probes the active vLLM miner and gateway before
...
Freya only lists Pearl-compatible models published by the Pearl Research
...
pass the Freya H100/H200 validation run.
