...
| **Overlay** | Sidecar file at `~/.freya/learning/skills/` storing optimized descriptions and few-shot examples |
...
freya skill install hermes:apple-notes

# Bulk install a category
freya skill sync hermes --category research

# Run a skill directly
freya skill run math-solver -a expression="41 + 82"

# See skill details
freya skill info research-and-summarize
```
...
freya skill sync hermes --category research
freya skill sync hermes --category coding
freya skill sync hermes  # everything (~150 skills)
```
...
freya skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
freya skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
freya skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
freya skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.freya/config.toml` for automatic syncing:
...
Freya can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
freya skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.freya/skills/discovered/
freya skill discover
```
...
freya optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
freya skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.freya/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
# Single condition
freya bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with Freya | Implicit trust | Full access within declared capabilities |
...
```bash
freya skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.freya/skills/"  # where skills are installed
...
2. **User** `~/.freya/skills/`
3. **Bundled** (shipped with Freya)

## CLI Reference

| Command | Description |
|---------|-------------|
| `freya skill list` | List installed skills |
| `freya skill info <name>` | Show detailed skill information |
| `freya skill run <name> [-a key=value]` | Execute a skill directly |
| `freya skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `freya skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `freya skill sources` | List configured skill sources |
| `freya skill update` | Pull latest from configured sources |
| `freya skill remove <name>` | Remove an installed skill |
| `freya skill search <query>` | Search the skill index |
| `freya skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `freya skill show-overlay <name>` | Inspect optimization output for a skill |
| `freya optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `freya bench skills [--condition C]` | Run the PinchBench skills benchmark |
