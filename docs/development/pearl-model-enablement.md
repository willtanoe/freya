...
through Pearl's vLLM miner and Freya.

Freya can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
Freya only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and Freya-specific conversion
...
`freya mine start`, vLLM `/v1/models`, Freya inference routing, Pearl
gateway template refresh, and `freya mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in Freya until we have clean
...
2. Convert the target model.
   ...
   Freya includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `freya mine inspect-model` and
   `freya mine validate-model` pass on H100/H200 hardware.

   Local staging artifacts can be inspected before upload:

   ```bash
   freya mine inspect-model \
...
3. Validate the Pearl vLLM plugin path.
   - Run `freya mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `freya mine status` parses those metrics.

5. Promote the model in Freya.
...
```text
src/freya/mining/_models.py
```

`freya mine models` renders that registry. Planned models are visible to users
...
- `freya mine inspect-model --model <pearl-model-id> --allow-planned`
- `freya mine init --model <pearl-model-id>`
- `freya mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `freya ask "Say hello in one sentence."`
- `freya mine status`
- `freya mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `freya mine validate-model --output` to the issue.
