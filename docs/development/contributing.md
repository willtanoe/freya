...
```bash
uv run freya --version   # Should print 0.1.0
uv run freya --help      # Show all subcommands
```
...
Freya uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=freya --cov-report=html
```
...
Freya uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/freya/`:
...
src/freya/
    ...
    core/                       # Shared infrastructure
        config.py               # FreyaConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # freya ask
        init_cmd.py             # freya init
        model.py                # freya model list/info
        memory_cmd.py           # freya memory index/search/stats
        telemetry_cmd.py        # freya telemetry stats/export/clear
        bench_cmd.py            # freya bench run
        serve.py                # freya serve
```
...
try:
    import freya.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
