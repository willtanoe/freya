...
use freya_core::{FreyaError, Trace};
...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, FreyaError> {
        ...
        let conn = Connection::open(db_path).map_err(|e| {
            FreyaError::Io(std::io::Error::other(e.to_string()))
        })?;
    ...
    pub fn save(&self, trace: &Trace) -> Result<(), FreyaError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, FreyaError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
    ...
    pub fn count(&self) -> Result<usize, FreyaError> {
        let conn = self.conn.lock();
        let count: i64 = conn
