...
use freya_core::config::FreyaConfig;
use freya_core::FreyaError;
...
pub fn discover_engines(config: &FreyaConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, FreyaError> {
    ...
    match key.as_str() {
        ...
        other => Err(FreyaError::Engine(
            freya_core::error::EngineError::ModelNotFound(format!(
...
mod tests {
    ...
    fn test_get_engine_static_ollama() {
        let config = FreyaConfig::default();
