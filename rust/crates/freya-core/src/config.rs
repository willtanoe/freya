...
//!
//! Rust translation of `src/freya/core/config.py`.
...
/// Default config directory: `~/.freya/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".freya")
}

/// Default config file path: `~/.freya/config.toml`
...
fn default_sandbox_image() -> String { "freya-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.freya/operators".into() }
...
pub fn load_config(path: Option<&Path>) -> Result<FreyaConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: FreyaConfig = toml::from_str(&content)?;
        ...
        let mut cfg = FreyaConfig::default();
        cfg.engine.default = recommended_engine;
        cfg
    };
...
mode = "block"
"#;
        let cfg: FreyaConfig = toml::from_str(toml_str).unwrap();
