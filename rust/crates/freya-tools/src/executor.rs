...
use freya_core::error::{FreyaError, ToolError};
use freya_core::{EventBus, EventType, ToolResult};
use freya_security::capabilities::CapabilityPolicy;
use freya_security::taint::{TaintSet, check_taint};
...
impl ToolExecutor {
    ...
    pub fn execute(
    ...
    ) -> Result<ToolResult, FreyaError> {
        let tool = self.tools.get(tool_name).ok_or_else(|| {
            FreyaError::Tool(ToolError::NotFound(tool_name.to_string()))
        })?;
        ...
        if let (Some(policy), Some(aid)) = (&self.capability_policy, agent_id) {
            ...
            for cap in &spec.required_capabilities {
                if !policy.check(aid, cap, "") {
                    return Err(FreyaError::Tool(ToolError::CapabilityDenied(
                        aid.to_string(),
        ...
        if let Some(taint_set) = taint {
            if let Some(violation) = check_taint(tool_name, taint_set) {
                return Err(FreyaError::Tool(ToolError::TaintViolation(
                    tool_name.to_string(),
                    violation,
                )));
            }
        ...
        if elapsed > timeout {
            ...
            return Err(FreyaError::Tool(ToolError::Timeout(
                timeout.as_secs_f64(),
                tool_name.to_string(),
            )));
        }
...
mod tests {
    ...
    fn test_executor_tool_not_found() {
        ...
        let err = exec
            ...
            .unwrap_err();
        assert!(matches!(err, FreyaError::Tool(ToolError::NotFound(_))));
    }
}
