...
pub struct PyWorkflowGraph {
    inner: freya_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: freya_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => freya_workflow::NodeType::Tool,
            "condition" => freya_workflow::NodeType::Condition,
            "parallel" => freya_workflow::NodeType::Parallel,
            "loop" => freya_workflow::NodeType::Loop,
            "transform" => freya_workflow::NodeType::Transform,
            _ => freya_workflow::NodeType::Agent,
        };
        let node = freya_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = freya_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<freya_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(freya_workflow::WorkflowBuilder::new(name)),
        }
    }
