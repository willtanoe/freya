...
Presets are installed via `freya init --preset <name>`, which copies
`configs/freya/examples/<name>.toml` to `~/.freya/config.toml`.
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `freya init` expects to be populated for a working first run.
