...
User configuration lives at ``~/.freya/config.toml``.  ``load_config()``
...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``FreyaConfig.mining``
...
DEFAULT_CONFIG_DIR = Path.home() / ".freya"
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e freya[learning-ace]`` once the
    ...
    # most Freya users have credentials for.
    ...
    # Empty string defaults to ``~/.freya/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "freya"
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.freya/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through Freya. You are not a cloud service. Respond "
...
class AnalyticsConfig:
    ...
    usage events sent to the Freya team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.freya/whatsapp_auth
    assistant_name: str = "Freya"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "freya-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.freya/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.freya/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.freya/SOUL.md"
    memory_path: str = "~/.freya/MEMORY.md"
    user_path: str = "~/.freya/USER.md"
    nudge_interval: int = 10
    persona_name: str = ""  # named persona dir under ~/.freya/personas/<name>/
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.freya/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/freya/skill-index.git"
    index_dir: str = "~/.freya/skill-index/"
    max_depth: int = 5
...
@dataclass
class FreyaConfig:
    """Top-level configuration for Freya."""
...
# Sections that users may set via ``freya config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(FreyaConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``FreyaConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import freya.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "FreyaConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> FreyaConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``FREYA_CONFIG`` when set,
        otherwise ``~/.freya/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = FreyaConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("FREYA_CONFIG"):
        config_path = Path(os.environ["FREYA_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``freya init``)
# ---------------------------------------------------------------------------
...
# Full reference config: freya init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.freya/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.freya/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.freya/whatsapp_auth
