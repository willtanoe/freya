...
from freya.evals.core.config import EvalConfigError, expand_suite, load_eval_config
...
class TestExpandSuite:
    ...
    def test_backend_from_benchmark(self):
        suite = EvalSuiteConfig(
            models=[ModelConfig(name="m1")],
            benchmarks=[BenchmarkConfig(name="gaia", backend="freya-agent")],
        )
        configs = expand_suite(suite)
        assert configs[0].backend == "freya-agent"
...
class TestCLIConfig:
    def test_run_missing_benchmark_and_config(self):
        ...
        from freya.evals.cli import main
    ...
    def test_run_config_loads_and_prints_suite_info(self, tmp_path):
        ...
        runner = CliRunner()
        with patch("freya.evals.cli._run_single", side_effect=Exception("mock")):
