"""CLI for the Freya evaluation framework."""
...
BACKENDS = {
    "freya-direct": "Engine-level inference (local or cloud)",
    "freya-agent": "Agent-level inference with tool calling",
...
def _build_backend(
    ...
    if backend_name == "freya-agent":
        ...
        from freya.evals.backends.external import OpenClawBackend
...
def _build_dataset(benchmark: str, subset: str | None = None):
    ...
    if benchmark == "supergpqa":
        ...
        from freya.evals.datasets.email_triage import EmailTriageDataset

        return EmailTriageDataset()
    elif benchmark == "morning_brief":
        from freya.evals.datasets.morning_brief import MorningBriefDataset

        return MorningBriefDataset()
    elif benchmark == "research_mining":
        from freya.evals.datasets.research_mining import ResearchMiningDataset

        return ResearchMiningDataset()
    elif benchmark == "knowledge_base":
        from freya.evals.datasets.knowledge_base import KnowledgeBaseDataset

        return KnowledgeBaseDataset()
    elif benchmark == "coding_task":
        from freya.evals.datasets.coding_task import CodingTaskDataset

        return CodingTaskDataset()
    elif benchmark == "loghub":
        from freya.evals.datasets.loghub import LogHubDataset
        ...
        from freya.evals.datasets.webchorearena import WebChoreArenaDataset

        return WebChoreArenaDataset()
    elif benchmark == "workarena":
        from freya.evals.datasets.workarena import WorkArenaDataset
        ...
        from freya.evals.datasets.security_scanner import SecurityScannerDataset
        ...
        from freya.evals.datasets.browser_assistant import BrowserAssistantDataset
        ...
        from freya.evals.datasets.liveresearchbench import (
...
def _build_scorer(benchmark: str, judge_backend, judge_model: str):
    ...
    if benchmark == "supergpqa":
        ...
        from freya.evals.scorers.swefficiency_structural import SWEfficiencyScorer
        ...
        from freya.evals.scorers.email_triage import EmailTriageScorer

        return EmailTriageScorer(judge_backend, judge_model)
    elif benchmark == "morning_brief":
        from freya.evals.scorers.morning_brief import MorningBriefScorer

        return MorningBriefScorer(judge_backend, judge_model)
    elif benchmark == "research_mining":
        from freya.evals.scorers.research_mining import ResearchMiningScorer

        return ResearchMiningScorer(judge_backend, judge_model)
    elif benchmark == "knowledge_base":
        from freya.evals.scorers.knowledge_base import KnowledgeBaseScorer

        return KnowledgeBaseScorer(judge_backend, judge_model)
    elif benchmark == "coding_task":
        from freya.evals.scorers.coding_task import CodingTaskScorer

        return CodingTaskScorer(judge_backend, judge_model)
    elif benchmark == "loghub":
        from freya.evals.scorers.loghub_scorer import LogHubScorer

        return LogHubScorer(judge_backend, judge_model)
    elif benchmark == "ama-bench":
        from freya.evals.scorers.ama_bench_judge import AMABenchScorer

        return AMABenchScorer(judge_backend, judge_model)
    elif benchmark == "lifelong-agent":
        from freya.evals.scorers.lifelong_agent_scorer import LifelongAgentScorer
        ...
        from freya.evals.scorers.webchorearena_scorer import WebChoreArenaScorer

        return WebChoreArenaScorer(judge_backend, judge_model)
    elif benchmark == "workarena":
        from freya.evals.scorers.workarena_scorer import WorkArenaScorer

        return WorkArenaScorer(judge_backend, judge_model)
    elif benchmark == "coding_assistant":
        from freya.evals.scorers.coding_assistant import CodingAssistantScorer
        ...
        from freya.evals.scorers.daily_digest import DailyDigestScorer

        return DailyDigestScorer(judge_backend, judge_model)
    elif benchmark == "doc_qa":
        from freya.evals.scorers.doc_qa import DocQAScorer

        return DocQAScorer(judge_backend, judge_model)
    elif benchmark == "browser_assistant":
        from freya.evals.scorers.browser_assistant import BrowserAssistantScorer
        ...
        from freya.evals.scorers.taubench import TauBenchScorer
        ...
        from freya.evals.scorers.liveresearchbench import (
...
def _build_judge_backend(judge_model: str, engine_key: str = "cloud"):
    ...
    try:
        return FreyaDirectBackend(engine_key=engine_key)
...
def _build_trackers(config) -> list:
    ...
    if getattr(config, "wandb_project", ""):
        try:
            from freya.evals.trackers.wandb_tracker import WandbTracker
    ...
    if getattr(config, "sheets_spreadsheet_id", ""):
        try:
            from freya.evals.trackers.sheets_tracker import SheetsTracker
...
def _run_terminalbench_native(config, console: Console) -> object:
    ...
    from freya.evals.backends.terminalbench_native import (
...
def _run_single(config, console: Optional[Console] = None) -> object:
    ...
    eval_backend = _build_backend(
        ...
        base_url=(
            ...
            or os.environ.get("FREYA_BACKEND_BASE_URL")
        ),
        api_key=(
            getattr(config, "api_key", None)
            or _metadata.get("api_key")
            or os.environ.get("FREYA_BACKEND_API_KEY")
        ),
    )
...
def _run_agentic(
    ...
    *,
    ...
    from freya.evals.core.agentic_runner import AgenticRunner
    from freya.evals.core.event_recorder import EventRecorder
    ...
    try:
        ...
        from freya.telemetry.session import TelemetrySession
    ...
    try:
        from freya.evals.core.export import export_hf_dataset
...
def _run_from_config(
    ...
    *,
    ...
    from freya.evals.core.config import expand_suite, load_eval_config
...
@click.option(
    ...
    help="Agent name for freya-agent backend",
)
...
def run(
    ...
    config = RunConfig(
        ...
        base_url=base_url or os.environ.get("FREYA_BACKEND_BASE_URL"),
        api_key=api_key or os.environ.get("FREYA_BACKEND_API_KEY"),
        metadata={
            "base_url": base_url or os.environ.get("FREYA_BACKEND_BASE_URL"),
            "api_key": api_key or os.environ.get("FREYA_BACKEND_API_KEY"),
        },
...
def run_all(
    ...
    for i, bench_name in enumerate(BENCHMARKS, 1):
        ...
        eval_backend = _build_backend("freya-direct", engine_key, "orchestrator", [])
...
def reparse_judge(jsonl_path, out_path, in_place, summary_out):
    ...
    from freya.evals.scorers.liveresearch import rescore_from_metadata
