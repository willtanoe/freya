...
Recipes are the universal composition format for Freya.  Each recipe
...
_USER_RECIPES_DIR = Path.home() / ".freya" / "recipes"
_USER_OPERATORS_DIR = Path.home() / ".freya" / "operators"
...
class Recipe:
    ...
    def to_builder_kwargs(self) -> Dict[str, Any]:
        """Convert recipe fields to kwargs for SystemBuilder/Freya.

        Returns a dict with only the non-None fields, keyed to match
        the SystemBuilder fluent API or Freya constructor parameters.
        """
        kwargs: Dict[str, Any] = {}
    ...
    def to_eval_suite(
        ...
        """
        from freya.recipes.composer import recipe_to_eval_suite
    ...
    def to_operator_manifest(self) -> Any:
        ...
        from freya.recipes.composer import recipe_to_operator
...
def discover_recipes(
    ...
    *,
    ...
    3. User ``~/.freya/recipes/`` directory
    4. User ``~/.freya/operators/`` directory
