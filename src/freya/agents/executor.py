...
from freya.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a FreyaSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after FreyaSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from freya.agents import AgentRegistry
        ...
        # Resolve engine + model from FreyaSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in FreyaSystem")
        model = (
            config.get("model")
        ...
        # agent's own ToolExecutor. Set by CLI paths like `freya agents ask`
        ...
        if self._system is not None:
            ...
            # agents, mirroring the one-shot `freya ask` path so they no
