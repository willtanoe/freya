"""Detect how Freya was installed so we can show the right upgrade
command (and run the right upgrade command for ``freya self-update``).

Three install paths are supported today:

- **PyPI** (``pip install freya``). The package lives somewhere
  inside ``site-packages``. Upgrade with ``pip install --upgrade freya``.
- **uv tool** (``uv tool install freya``). Lives in a uv-managed
  isolated venv under ``~/.local/share/uv/tools/``. Upgrade with
  ``uv tool upgrade freya``.
...
We detect by inspecting ``freya.__file__``. If we can't tell with
...
def detect_install() -> InstallInfo:
    ...
    Cheap: just walks the parent directories of ``freya.__file__``
    once and checks for marker directories. No subprocess calls.
    """
    try:
        import freya

        pkg_file = Path(freya.__file__).resolve()
    except Exception:
        return InstallInfo(
            kind="unknown",
            upgrade_command="pip install --upgrade freya",
        )
