"""`freya self-update` — upgrade Freya to the latest release.

Runs the right upgrade command for how the user installed Freya:

- PyPI installs get ``pip install --upgrade freya``.
- uv-tool installs get ``uv tool upgrade freya``.
...
@click.command(
    "self-update",
    help=(
        "Upgrade Freya to the latest release. Detects how you "
...
def self_update(check: bool, yes: bool) -> None:
    ...
    click.echo(f"Current Freya version: v{current}")
    ...
    click.echo("\nUpgrade complete. Re-run `freya --version` to confirm.")
