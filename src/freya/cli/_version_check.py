"""Check for newer Freya releases on PyPI."""
...
_CACHE_PATH = Path("~/.freya/version-check.json").expanduser()
_CACHE_TTL = 86400  # 24 hours
_PYPI_API = "https://pypi.org/pypi/freya/json"


def _config_path() -> Path:
    """Resolve the config path, honoring ``FREYA_CONFIG`` like core.config."""
    override = os.environ.get("FREYA_CONFIG")
    if override:
        return Path(override).expanduser()
    return Path("~/.freya/config.toml").expanduser()
...
# - ``FREYA_NO_UPDATE_CHECK=1`` — project-specific
# - ``CI=true`` — set by every major CI provider, suppresses by default
_OPT_OUT_ENV_VARS = ("FREYA_NO_UPDATE_CHECK",)
...
def _check_disabled() -> bool:
    ...
    # ``FREYA_NO_UPDATE_CHECK=0`` if they want the nudge anyway.
...
def check_for_updates(command_name: str) -> None:
    ...
    Honors ``FREYA_NO_UPDATE_CHECK=1`` and ``CI=true`` — any
...
def _do_check() -> None:
    ...
    try:
        if Version(latest) > Version(current):
            from freya.cli._install_detect import detect_install

            cmd = detect_install().upgrade_command
            sys.stderr.write(
                f"\033[33mA new version of Freya is available "
                f"(v{current} → v{latest})\n"
                f"Update: {cmd}\n"
                f"Or run: freya self-update\033[0m\n\n"
            )
