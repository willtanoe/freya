"""``freya init`` — detect hardware, generate config, write to disk."""
...
from freya.cli._banner import print_banner
from freya.cli._bootstrap import detect_cloud_keys
from freya.cli.model import find_model_spec, hf_download, ollama_pull
...
# Engines supported by ``freya init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     freya ask "Hello"\n'
            "\n"
            "  Run `freya doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]freya scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-freya first-run guard.",
)
@click.pass_context
def init(
    ctx: click.Context,
    ...
    from_bare_freya: bool = False,
) -> None:
    """Detect hardware and generate ~/.freya/config.toml."""
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "freya" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]freya doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-freya cold path: use the recommended engine non-interactively.
        if from_bare_freya:
            engine = recommended
        else:
            console.print()
...
"""
        ...
        console.print(
            ...
            "Run [bold]freya connect gdrive[/bold] to connect "
            "Google services, then [bold]freya digest --fresh[/bold]."
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_freya and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]freya model pull {model}[/bold]"
                )
