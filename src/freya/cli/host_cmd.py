"""``freya host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      freya host mlx-community/Qwen2.5-7B-4bit --backend mlx
      freya host Qwen/Qwen3-8B --backend vllm
      freya host qwen3:8b --backend ollama
      freya host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]Freya will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
