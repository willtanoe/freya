"""``freya start|stop|restart|status`` — daemon management commands."""
...
from freya.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the Freya server daemon."""
...
def start(
    ...
    """Start the Freya server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'freya stop' to stop it first, or 'freya restart'.")
        sys.exit(1)
    ...
    # Build command to run freya serve
    cmd = [sys.executable, "-m", "freya.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Freya server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running Freya server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the Freya server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the Freya server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
