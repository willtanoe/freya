"""``freya scan`` — audit your environment for privacy and security risks."""
...
class PrivacyScanner:
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.freya is inside iCloud Drive sync scope."""
        try:
            config_path = Path("~/.freya").expanduser().resolve()
            icloud_path = Path("~/Library/Mobile Documents/").expanduser().resolve()
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.freya may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.freya is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]Freya Security Scan[/bold]")
    console.print()
    ...
    if fail_count:
        console.print(
            ...
            "before storing sensitive data with Freya."
        )
        console.print()
    elif warn_count:
        console.print(
