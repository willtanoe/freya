"""``freya scheduler`` -- task scheduling commands."""
...
def scheduler_run_task(agent_name: str, dry_run: bool) -> None:
    ...
    when Freya is not running as a persistent daemon.

    Example (launchd plist ProgramArguments):
        freya scheduler run-task proactive
    """
    console = Console()
    store = _get_store()
    try:
        ...
        if match is None:
            console.print(
                ...
                "Register it first with 'freya scheduler create'.[/yellow]"
            )
            return
