"""``freya doctor`` — run diagnostic checks on the Freya installation."""
...
from freya.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `freya init` to generate a config file.",
    )
...
def _check_engines() -> List[CheckResult]:
    ...
    from freya.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "freya[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "freya[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "freya[energy-amd]", "AMD energy monitoring"),
        ("colbert", "freya[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "freya[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Freya installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Freya Doctor[/bold]")
    console.print()
    ...
    from freya.cli._bg_state import get_status
    ...
    if bg.rust_extension == "ready":
        ...
        console.print(
            "    retry: ~/.freya/.scripts/install-rust.sh && "
            "~/.freya/.scripts/build-extension.sh"
        )
        bg_failed = True
    else:
        console.print(
    ...
    for model_id, state in bg.models.items():
        if state == "ready":
            ...
            console.print(f"    retry: ~/.freya/.scripts/pull-model.sh {model_id}")
            bg_failed = True
        else:
