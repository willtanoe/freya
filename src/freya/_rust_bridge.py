"""Single point of contact between Python and the Rust ``freya_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``freya_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    ...
    """
    import freya_rust  # type: ignore[import-untyped]

    return freya_rust


def _detect_rust() -> bool:
    """Return ``True`` if the compiled ``freya_rust`` extension is importable.
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from freya.security.injection_scanner import (
        InjectionScanResult,
    )
    from freya.security.types import ScanFinding, ThreatLevel
