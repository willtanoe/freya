...
Uses GEPA's adapter pattern to bridge Freya traces into GEPA's
...
from freya.core.registry import LearningRegistry
from freya.learning._stubs import AgentLearningPolicy
...
class FreyaGEPAAdapter:
    """Implements GEPA's adapter protocol for Freya agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'freya[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = FreyaGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "FreyaGEPAAdapter"]
