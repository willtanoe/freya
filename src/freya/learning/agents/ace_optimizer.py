...
The wrapper adapts Freya traces into ACE's
...
sidecar overlay under ``~/.freya/learning/ace/<task>/`` for the
...
def _default_save_dir(task_name: str) -> Path:
    return Path.home() / ".freya" / "learning" / "ace" / task_name


class _TraceDataProcessor:
    """Adapter that exposes Freya traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert Freya trace records to ACE's sample dict shape."""
