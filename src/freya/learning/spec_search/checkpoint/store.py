...
A thin wrapper over a local git repository at ``<freya_home>/.git``.
...
from freya.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.freya/`` (or ``$FREYA_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Freya source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Freya source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@freya.local")
            self._git("config", "user.name", "Freya Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new freya home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
