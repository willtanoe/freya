"""FastAPI application factory for the Freya API server."""
...
from freya.server.analytics_routes import router as analytics_router
from freya.server.api_routes import include_all_routes
from freya.server.comparison import comparison_router
from freya.server.connectors_router import create_connectors_router
from freya.server.dashboard import dashboard_router
from freya.server.digest_routes import create_digest_router
from freya.server.research_router import router as research_router
from freya.server.routes import router
from freya.server.upload_router import router as upload_router
...
def create_app(
    ...
    *,
    ...
    config:
        Optional FreyaConfig for other settings.
    """
    app = FastAPI(
        title="Freya API",
        description="OpenAI-compatible API server for Freya",
        version="0.1.0",
    )
    ...
    try:
        from freya.server.middleware import create_security_middleware
    ...
    if webhook_config:
        try:
            from freya.server.webhook_routes import (
