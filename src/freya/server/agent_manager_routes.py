...
logger = logging.getLogger("freya.server.agent_manager")
...
class _LightweightSystem:
    ...
    full FreyaSystem (which picks a random model from Ollama)."""
...
def _make_lightweight_system(
    ...
    try:
        ...
        try:
            from freya.core.events import get_event_bus
...
def _ensure_registries_populated() -> None:
    ...
    from freya.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import freya.channels  # noqa: F401
    except Exception:
        pass
    ...
    # Also try to import browser tools (not included in freya.tools.__init__)
    for _browser_mod in ("freya.tools.browser", "freya.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("freya.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from freya.core.credentials import TOOL_CREDENTIALS
    from freya.core.registry import ChannelRegistry, ToolRegistry
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from freya.core.types import ToolCall as MsgToolCall
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from freya.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from freya.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from freya.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from freya.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from freya.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from freya.core.events import EventType, get_event_bus
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.freya/traces.db")
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from freya.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from freya.core.credentials import get_credential_status
