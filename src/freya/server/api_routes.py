...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import freya.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from freya.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from freya.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from freya.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from freya.tools.agent_tools import AgentSendTool
...
async def list_skills(request: Request):
    ...
    try:
        from freya.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.freya/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from freya.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP freya_requests_total Total requests processed",
            "# TYPE freya_requests_total counter",
            f"freya_requests_total {stats.get('total_requests', 0)}",
            "# HELP freya_tokens_total Total tokens generated",
            "# TYPE freya_tokens_total counter",
            f"freya_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP freya_latency_avg_ms Average latency in milliseconds",
            "# TYPE freya_latency_avg_ms gauge",
            f"freya_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from freya.server.auth_middleware import websocket_authorized
...
def include_all_routes(app) -> None:
    ...
    from freya.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from freya.server.agent_manager_routes import (  # noqa: PLC0415
