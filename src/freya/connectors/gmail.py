...
Uses OAuth 2.0 tokens stored locally (see :mod:`freya.connectors.oauth`).
...
from freya.connectors.google_auth import (
    GoogleAuthError,
)
from freya.connectors.google_auth import (
...
class GmailConnector(BaseConnector):
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `freya connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `freya connect gmail`"
            )
        return new
