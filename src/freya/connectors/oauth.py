...
class OAuthProvider:
    ...
    # Filenames in ~/.freya/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.freya/connectors/`` and falls
    back to environment variables ``FREYA_{NAME}_CLIENT_ID`` and
    ``FREYA_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"FREYA_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Freya.</p>"
                    b"</body></html>"
                )
